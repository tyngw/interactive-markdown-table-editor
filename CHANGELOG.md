...
## 0.9.5
- エディタ全体の右クリックメニューに `Export CSV` / `Export TSV` を追加し、UTF-8 / Shift_JIS をチェック形式で切り替えられるようにしました。
- エクスポート専用ツールバーを廃止し、シートタブをステータスバー直上に再配置して表示領域を広げました。
- WebviewManager 周辺の未使用コードを整理し、初期化・ヘルスチェック処理を最小限にしました。
- タブバーとステータスバーを１つのボトムコンテナにまとめ、テーブル領域との不要な空白を解消しました。
...
- 編集モード中に Ctrl/Cmd+Z でもとに戻す、Shift+Ctrl/Cmd+Z または Ctrl/Cmd+Y でやり直しに対応
- 通常のセル選択モードでは VS Code の Undo/Redo を使用して確実に元に戻す/やり直しを実行
...
### Fixed
- すでにエディタを開いている状態で、別のファイルを開いた際に、エディタの内容が正しく更新されない問題を修正しました。
