...
## 1.3.6
- ライブラリアップデート
  - Dependabot の依存関係更新 PR を反映
  - `npm audit` で検出された開発依存の脆弱性を追加で解消
...
- 新機能
  - 複数テーブルのタブ表示を強化
    - **タブラベルに見出しを使用**：テーブル直前の Markdown 見出しをタブ名として表示するモードを追加。設定 `markdownTableEditor.tabLabelMode` で `"number"`（従来の「表 1」形式）と `"heading"`（見出し名）を切り替え可能。同一見出し下に複数テーブルがある場合は「見出し (1)」「見出し (2)」のようにサフィックスを付与。
    - **ハンバーガーメニュー**：タブバー左端に ☰ ボタンを追加。クリックで全テーブルの一覧がドロップダウン表示され、素早くタブを切り替え可能。表示領域が狭い場合は自動的に上方向に開く。
    - **ツールチップ**：長いタブラベルは省略表示（…）し、1 秒ホバーで全文をツールチップ表示。
- ライブラリアップデート

## 1.3.3
- バグ修正
  - 拡張機能マニフェストの VS Code 互換性指定を固定値からレンジ指定へ修正し、Cursor / VS Code の新しい 1.x 系バージョンでも開発中拡張機能を読み込めるように改善

## 1.3.2
- 新機能
  - Markdown フォーマッティングのレンダリング機能：セル内での markdown フォーマット（bold, italic等）の表示に対応
- バグ修正
- ライブラリアップデート

## 1.3.1
- 改善
  - コンテキストメニューの「テーブルエディターを開く」エントリを 1つに統一（「新しいパネルで開く」エントリを削除）
  - 拡張機能設定に下記オプションを追加
    - `markdownTableEditor.openInNewPanel`：常に新しいパネルで開くかどうかを事前設定（デフォルト: false）
    - `markdownTableEditor.panelPosition`：パネルの表示位置を `beside`（サイド表示）または `active`（同じ列に全面表示）から選択可能（デフォルト: beside）
 - 新機能
   - テーブル編集時の余白調整と自動フィット（auto-fit）機能を強化し、表示・編集時のレイアウト安定性を向上
 - バグ修正
   - カラムリサイズ中にマウスボタンがパネル外で離された場合の終了判定を追加
   - Git差分検出ロジックを修正し、差分の検出精度を改善
   - 連続する削除・追加が混在する差分表示での表示順序を修正
   - キーボードナビゲーションで先頭行へ移動する際にコンテナのスクロール位置が正しく調整されない問題を修正（`container.scrollTop = 0` を明示的に設定）

## 1.3.0
- 新機能
  - テーブルが存在しない Markdown ファイルでエディターを開く際に、空のテーブルを作成するダイアログを表示
    - 「テーブルを作成」ボタンを選択すると列数・行数の入力を促す
    - 入力した列数（1〜100）・行数（1〜1000）の空テーブルをカーソル位置またはファイル末尾に自動挿入
    - 挿入後にテーブルエディターを起動
    - 英語・日本語・中国語（簡体字）の多言語に対応

## 1.2.0
- 新機能
  - 自動保存のON/OFF切り替え機能を追加
    - ステータスバーに「Save」ボタンを追加（Git Diffボタンと同様のUI）
    - 自動保存OFFの場合、未保存の変更がある場合にウィンドウタイトルに「*」マークを表示
    - Ctrl+S（Mac: Cmd+S）で手動保存が可能
  - ユニットテストとカバレッジ支援機能を追加
    - 包括的なユニットテストスイートを実装
    - NYC（New York Code coverage）を使用したテストカバレッジ計測
    - カバレッジレポートの自動生成と PR コメント機能
- 改善
  - 複数行・列の移動機能を拡張
    - 選択した行数・列数に応じた複数行・列の一括移動に対応
  - 複数セル選択時の選択境界スタイルを改善
    - 非アンカーセルの選択境界をより明確に表示
  - Git差分表示の正確性を向上
    - Git API（status, diffWithHEAD）を使用した差分取得と適用
  - ヘッダーおよび行番号セルのスタイルを統一
    - 共通スタイルの抽出と適用
- バグ修正
  - ヘッダーコーナーのアイコンを削除
  - テスト環境での vscode API モック化の問題を修正
  - CI テストの実行安定性を向上
    - TypeScript コンパイル ステップの追加
    - NYC カバレッジ計測の Electron プロセス対応
- テスト・CI
  - GitHub Actions ワークフローの改善
    - Dependabot PR への自動テスト実行対応
    - カバレッジレポート生成と PR コメント機能
  - 依存関係の更新
    - markdown-it を 13.0.2 から 14.1.1 に更新
    - qs を 6.14.1 から 6.14.2 に更新

## 1.1.3
- 改善
  - ヘッダーの垂直揃えとパディングを調整
  - テーマ変数の参照を更新し、テーブルヘッダーと行番号の表示を改善
  - Webviewのグローバルスタイルをインライン化し、配信を最適化
  - ヘッダーおよびセルのIME処理を改善
  - キーボード操作時のスクロールパディングを修正
  - 検索・置換の入力欄を正しく表示
  - 部分的なセル更新に対応
  - スコアベースのマッチングによるGit差分表示の列対応精度を向上
  - Git差分計算を非同期化し、UI応答性を改善
- リファクタリング
- テスト・CI
...
## 1.1.2
- バグ修正
  - Git差分表示で削除行の行番号列マッピングとレイアウト調整を修正
  - 同一コンテンツセルが正しく同一コンテンツクラスでレンダリングされるように修正
  - Git差分の削除セルで改行を正しく保持するように修正
- テスト基盤の改善と統合テストの改善
  - CSV インポート時の UTF-8 および Shift-JIS エンコーディング対応のテストを追加
  - CSV エクスポート統合テストの最適化
  - E2E テストの強化
- 列差分検出機能の改善
  - Git 差分表示で削除行にサムネイル背景色を追加
  - 列差分検出モジュールを実装（セルパーサー、ヘッダー比較ユーティリティを含む）
- リファクタリング
  - テストファイル構成の整理（`test/suite` → `test/unit`）
  - Webview配信ディレクトリを最適化（`webview-dist/` → `out/webview/`）
  - 不要なファイルの削除
  - テストディレクトリ構成の再編

## 1.1.1
- vscodeのバージョン制約を緩和（^1.70.0）
- ログ出力の最適化

## 1.1.0
- Git差分表示機能を追加
- 横スクロール時に行番号列が固定されるようになりました
  - `position: sticky; left: 0;` の CSS スタイルを適用して、行番号セルを横スクロール時も固定表示
  - `z-index` を調整してヘッダーと行番号列の表示順序を適切に管理
- WebviewのスタイリングシステムをEmotionに移行

## 1.0.6
- 選択操作の改善
- ドラッグによるセル選択機能を再実装。
  - `isSelecting`状態を削除し、`selectCell`関数を再利用することでロジックを簡素化・安定化。
  - Shiftキーと同様の挙動（アンカー起点の範囲選択）をドラッグ操作でも実現。
- オートフィル機能の修正
  - フィル範囲のスタイル（透明背景・点線ボーダー）を調整。

## 1.0.5
- 編集モードに入らずに文字を入力した際、Macのライブ変換などで途中まで入力した文字列が消えてしまう問題を修正。

## 1.0.4
- テーブル編集時にMarkdownの区切り線（`| --- | --- |`）が書き変わってしまう問題を修正。
  - パース時にオリジナルの区切り線を`TableNode`と`TableData`に保存するように変更。
  - シリアライズ時に保存された区切り線を使用し、オリジナルのフォーマットを保持。
  - 列の追加・削除・移動時にも区切り線を適切に更新。
  - CSVインポート等で内容を置換する場合は区切り線をクリア。

## 1.0.3
- パイプ文字（`|`）の処理方法を改善し、貼り付け後のセパレータ誤認識問題を修正。
  - Webview側でのエスケープ処理を削除し、Extension側（Markdown出力時）でエスケープするように変更。
  - `serializeToMarkdown`メソッドでセル内容のパイプ文字を自動的に`\|`にエスケープ。
  - 既にエスケープされているパイプ文字の二重エスケープを防止。
  - データの往復（貼り付け→保存→読み込み）で一貫性を保証。

## 1.0.2
- WebViewを開いている状態で別のファイルの右クリックメニューからテーブルエディタを開いた際に、前回のファイルの内容のままで新たに開いたファイルの内容がWebViewに反映されない問題を修正。
  - 既存のパネルを再利用する際に、`communicationManagers`のマッピングとハンドラーが更新されていなかったため、新しいファイルの内容が反映されていなかった。
  - 既存のパネルを再利用する際に、古い通信マネージャーを破棄し、新しいURIで新しい通信マネージャーを作成するように修正。
  - `setupCommunicationHandlers`を再実行して、新しいURIでハンドラーを設定するように修正。

## 1.0.1
- パイプ文字（`|`）を含むデータの貼り付け時に、セパレータとして誤解釈されないようエスケープ処理を追加。
  - クリップボードから貼り付けた際、セル内の`|`文字を自動的に`\|`にエスケープ。
  - 表示時と編集時には`\|`を`|`として正しく表示。
  - Markdownテーブルのセパレータとの競合を回避。

## 1.0.0
- 正式リリース版として公開。

## 0.13.1
- パフォーマンス改善：セル単位でのメモ化を実施。
  - 新しい `MemoizedCell` コンポーネントを追加し、`React.memo` でラップ。
  - 大量データ時のセル選択操作のパフォーマンスを向上。
  - 選択状態が変更されても、影響を受けないセルは再レンダリングをスキップ。
  - カスタム比較関数により、効率的な変更検知を実現。
- セル編集時のパフォーマンス最適化。
  - CellEditor での DOM操作を削減（スペーサーDIV削除）。
  - セルの高さを編集セルに直接設定し、複雑なスペーサーシステムを廃止。
  - 大量データテーブルで編集セルが行内で最も高い場合のパフォーマンスが向上。
- ヘッダー行（row=-1）のペースト機能を改善。
  - 列ヘッダーがOFFの場合、ペーストで見出し行を更新可能に。
  - 重複ペースト実行の問題を修正。
  - ヘッダー行の座標検証と更新ロジックを統一。
- キーボード操作の改善。
  - 複数セル選択時の Enter キー動作を改善。
  - Enterで複数選択範囲内のアンカーセルをサイクル（下移動、右移動、ラップアラウンド）。
  - 単一セル選択では従来通り編集モードに入る仕様は変わらず。
  - F2で常に編集モードに入るオプションを保持。
  - アンカーセルの選択状態をEnter入力時に維持。
- セル選択UIの改善。
  - 選択範囲の視覚的フィードバックを強化。
  - アンカーセル（開始地点）の背景色を濃くして視認性向上（alpha 0.15 vs 0.3）。
  - 複数セル選択時、選択範囲の内側に枠線を表示（外側の重複防止）。
  - シングルセル選択では背景色を非表示にし、枠線のみ表示。
  - ズインデックス（z-index）の調整により、スティッキーヘッダー下に隠れることなく表示。
  - フィルハンドルの表示位置を改善し、隣接セルで隠れない対策。
- 列リサイズハンドルの改善。
  - リサイズハンドルを列境界の中央に配置（視認性向上）。
  - セル内容との境界が明確に。
- IME入力時の ペースト処理の改善。
  - 入力キャプチャ要素（input-capture）の onPaste ハンドラーで EditMode遷移を防止。
  - キーボード操作での Edit Mode への遷移は useKeyboardNavigation で統一。
- ドラッグ＆ドロップ機能の改善。
  - ドラッグプレビューの位置計算をスクロールオフセットに対応。
  - 行・列移動時の座標計算がより正確に。

## 0.13.0
- CSVインポート機能を修正・改善。
  - 右クリックメニューからのCSVインポートが動作しない問題を修正。
  - 通信プロトコル層にIMPORT_CSVコマンドを追加し、完全に統合。
  - CSVインポートダイアログとエラーメッセージを多言語化。
  - ファイル選択ダイアログにタイトルを追加（日本語・中国語対応）。
  - 包括的なテストスイート（プロトコル、バリデータ、通信マネージャー、UI統合）を追加。
- 列ヘッダのダブルクリック編集機能を修正。
  - 列選択機能追加後にダブルクリック編集が動作しなくなっていた問題を修正。
  - シングルクリック（250ms遅延）で列選択、ダブルクリックで編集モードに入るように改善。
  - 列ヘッダがOFFの場合はダブルクリック編集を無効化。
- 列選択機能を追加。
  - 列ヘッダをクリックして列全体を選択可能に。
  - Shiftキーでの範囲選択に対応。
  - 選択した列数分の列を一度に追加可能に（「この左に3列を追加」など）。
- 複数行・複数列の追加機能を強化。
  - 全選択された行数/列数に応じて一度に複数行/列を追加可能に。
  - countパラメータを通信レイヤー全体で適切に渡すように修正。
  - フロントエンドとバックエンド間の型安全性を向上。
- ダークモードでのコンテキストメニュー表示を改善。
  - ホバー時の背景色を調整し、視認性を向上。
- 国際化（i18n）サポートを拡張。
  - テーブルラベルとハードコードされた文字列をすべて多言語化。
  - 日本語・中国語の翻訳を追加・改善。
- IME入力対応を改善。
  - IME入力中でもセル編集モードが維持されるように修正。
- 内部リファクタリング。
  - セルDOM操作ユーティリティを抽出・集約。
  - 通信レイヤーの型安全性を向上。
  - 開発用サーバーと古いドキュメントを削除。
