...
## 0.9.5
- Undo/Redo 対応を含むテーブル操作を `runTableEdit` ヘルパーに集約し、行・列追加／削除処理の重複と抜け漏れを排除。
- `fileHandler` が VS Code のオープンドキュメントを直接編集するよう変更し、未保存変更を含む最新内容を常に反映。
- ソート適用時でもセル編集・貼り付け・切り取り・行削除が正しい行に反映されるよう、Webview 側でビュー行からモデル行へのマッピングを共通化。
